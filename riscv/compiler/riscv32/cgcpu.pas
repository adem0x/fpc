{
    Copyright (c) 1998-2014 by Florian Klaempfl, David Zhang, and Jeppe Johansen

    This unit implements the code generator for RISC-V.
    Adapted from the MIPS CG code
  ...
  implementation
    ...
    const
      TOpcg2AsmOp: array[TOpCg] of TAsmOp = (
        A_NONE,A_NONE,A_ADD,A_AND,A_NONE,A_NONE,A_NONE,A_NONE,A_NONE,A_NONE,
        A_OR,A_SRA,A_SLL,A_SRL,A_SUB,A_XOR,A_NONE,A_NONE
      );
    ...
    procedure tcgriscv.make_simple_ref(list: tasmlist; var ref: treference);
      var
      ...
      begin
        ...
        if (ref.symbol=nil) and
           (ref.offset>=simm12lo) and
           (ref.offset<=simm12hi-sizeof(pint)) then
          exit;
        ...
        if assigned(ref.symbol) then
          begin
            ...
            if (tmpreg<>NR_NO) then
              list.concat(taicpu.op_reg_reg_reg(A_ADD,ref.base,tmpreg,ref.base));

            ref.symbol:=nil;
            if (ref.offset=0) then
              exit;

            if (ref.offset>=simm12lo) and
              (ref.offset<=simm12hi-sizeof(pint)) then
              begin
                list.concat(taicpu.op_reg_reg_const(A_ADDI,ref.base,ref.base,ref.offset));
                ref.offset:=0;
                exit;
              end;
        ...
        if (ref.base=NR_NO) then
          ...
          begin
            if (not base_replaced) then
              ref.base:=getintregister(list,OS_ADDR);
            list.concat(taicpu.op_reg_reg_reg(A_ADD,ref.base,tmpreg,tmpreg1))
          end;
        ref.offset:=0;
      end;


    procedure tcgriscv.handle_reg_const_reg(list: tasmlist; op: Tasmop; src: tregister; a: tcgint; dst: tregister);
      var
        tmpreg: tregister;
      begin
        if op=A_SUB then
    ...
    procedure tcgriscv.init_register_allocators;
      begin
        inherited init_register_allocators;

        rg[R_INTREGISTER] := trgintcpu.Create(R_INTREGISTER, R_SUBD,
          [RS_X26,RS_X27,RS_X28,RS_X29,RS_X30,
           RS_X3,RS_X4,RS_X5,RS_X6,RS_X7,RS_X8,RS_X9,RS_X10,RS_X11,RS_X12,RS_X13,
           RS_X16,RS_X17,RS_X18,RS_X19,RS_X20,RS_X21,RS_X22,RS_X23,RS_X24,RS_X25,
           RS_X2],
          first_int_imreg, []);
        ...
        rg[R_MMREGISTER]:=trgcpu.create(R_MMREGISTER,R_SUBNONE,
            [RS_X0],first_mm_imreg,[]);
      end;


    procedure tcgriscv.done_register_allocators;
      begin
    ...
    procedure tcgriscv.a_loadfpu_ref_cgpara(list: tasmlist; size: tcgsize; const ref: treference; const paraloc: TCGPara);
      var
    ...
    procedure tcgriscv.a_loadfpu_reg_cgpara(list: tasmlist; size: tcgsize; const r: tregister; const paraloc: TCGPara);
      var
        href: treference;
      begin
    ...
    procedure tcgriscv.a_call_name(list: tasmlist; const s: string; weak: boolean);
      var
        sym: tasmsymbol;
      begin
        ...
        list.concat(taicpu.op_reg_sym(A_JAL,NR_RETURN_ADDRESS_REG,sym));
      end;


    procedure tcgriscv.a_call_reg(list: tasmlist; Reg: TRegister);
      begin
        if assigned(current_procinfo) and
           not (pi_do_call in current_procinfo.flags) then
          InternalError(2013022102);

        list.concat(taicpu.op_reg_reg_const(A_JALR,NR_RETURN_ADDRESS_REG,reg,0));
      end;


    procedure tcgriscv.a_load_const_reg(list: tasmlist; size: TCGSize; a: tcgint; reg: TRegister);
      begin
        if (a >= simm12lo) and (a <= simm12hi) then
          list.concat(taicpu.op_reg_reg_const(A_ADDI, reg, NR_X0, a))
        else
          begin
            list.concat(taicpu.op_reg_const(A_LUI, reg, aint(a) shr 12));
            if (a and aint($FFF))<>0 then
              list.concat(taicpu.op_reg_reg_const(A_ORI,reg,reg,a and aint($FFF)));
          end;
      end;


    procedure tcgriscv.a_load_const_ref(list: tasmlist; size: tcgsize; a: tcgint; const ref: TReference);
      begin
        if a = 0 then
    ...
    procedure tcgriscv.a_load_reg_ref(list: tasmlist; FromSize, ToSize: TCGSize; reg: tregister; const Ref: TReference);
      var
        op: tasmop;
        href: treference;
      begin
    ...
    procedure tcgriscv.a_load_ref_reg(list: tasmlist; FromSize, ToSize: TCgSize; const ref: TReference; reg: tregister);
      var
        op: tasmop;
        href: treference;
      begin
    ...
    procedure tcgriscv.a_load_reg_reg(list: tasmlist; fromsize, tosize: tcgsize; reg1, reg2: tregister);
      var
        instr: taicpu;
        done: boolean;
      begin
        ...
        begin
          ...
          case tosize of
            ...
            begin
              ...
              list.concat(taicpu.op_reg_reg_const(A_SRLI, reg2, reg2, 16));
            end;
            OS_32,
            OS_S32:
              done:=false;
    ...
    procedure tcgriscv.a_loadaddr_ref_reg(list: tasmlist; const ref: TReference; r: tregister);
      var
        href: treference;
        hreg: tregister;
      begin
        ...
        if (ref.symbol=nil) then
          begin
            if (ref.base<>NR_NO) then
              begin
                if (ref.offset<simm12lo) or (ref.offset>simm12hi) then
                  begin
                    hreg:=getintregister(list,OS_INT);
                    a_load_const_reg(list,OS_INT,ref.offset,hreg);
                    list.concat(taicpu.op_reg_reg_reg(A_ADD,r,ref.base,hreg));
                  end
                else if (ref.offset<>0) then
                  list.concat(taicpu.op_reg_reg_const(A_ADDI,r,ref.base,ref.offset))
                else
                  a_load_reg_reg(list,OS_INT,OS_INT,ref.base,r);  { emit optimizable move }

                if (ref.index<>NR_NO) then
                  list.concat(taicpu.op_reg_reg_reg(A_ADD,r,r,ref.index));
              end
            else
        ...
        { Add original base/index, if any. }
        if (ref.base<>NR_NO) then
          list.concat(taicpu.op_reg_reg_reg(A_ADD,r,r,ref.base));
        if (ref.index<>NR_NO) then
          list.concat(taicpu.op_reg_reg_reg(A_ADD,r,r,ref.index));
        ...
        {if (ref.offset<simm12lo) or (ref.offset>simm12hi) then
          begin
            hreg:=getintregister(list,OS_INT);
            a_load_const_reg(list,OS_INT,ref.offset,hreg);
            list.concat(taicpu.op_reg_reg_reg(A_ADD,r,r,hreg));
          end
        else if (ref.offset<>0) then
          list.concat(taicpu.op_reg_reg_const(A_ADDI,r,r,ref.offset));}
      end;


    procedure tcgriscv.a_loadfpu_reg_reg(list: tasmlist; fromsize, tosize: tcgsize; reg1, reg2: tregister);
      var
        instr: taicpu;
      begin
    ...
    procedure tcgriscv.a_loadfpu_ref_reg(list: tasmlist; fromsize, tosize: tcgsize; const ref: TReference; reg: tregister);
      var
        href: TReference;
      begin
        {href:=ref;
    ...
    procedure tcgriscv.a_loadfpu_reg_ref(list: tasmlist; fromsize, tosize: tcgsize; reg: tregister; const ref: TReference);
      var
        href: TReference;
      begin
        {if tosize<>fromsize then
    ...
    procedure tcgriscv.maybeadjustresult(list: TAsmList; op: TOpCg; size: tcgsize; dst: tregister);
      const
    ...
    procedure tcgriscv.overflowcheck_internal(list: tasmlist; arg1, arg2: tregister);
      var
    ...
    procedure tcgriscv.a_op_const_reg(list: tasmlist; Op: TOpCG; size: tcgsize; a: tcgint; reg: TRegister);
      begin
    ...
    procedure tcgriscv.a_op_reg_reg(list: tasmlist; Op: TOpCG; size: TCGSize; src, dst: TRegister);
      begin
        case Op of
          OP_NEG:
            list.concat(taicpu.op_reg_reg_reg(A_SUB, dst, NR_X0, src));
          OP_NOT:
            list.concat(taicpu.op_reg_reg_const(A_XOR, dst, src, -1));
          {OP_IMUL,OP_MUL:
            begin
    ...
    procedure tcgriscv.a_op_const_reg_reg(list: tasmlist; op: TOpCg; size: tcgsize; a: tcgint; src, dst: tregister);
      var
        l: TLocation;
      begin
    ...
    procedure tcgriscv.a_op_reg_reg_reg(list: tasmlist; op: TOpCg; size: tcgsize; src1, src2, dst: tregister);
      begin
          ...
          begin
            if (size in [OS_S8,OS_S16]) then
              begin
                ...
                list.concat(taicpu.op_reg_reg_const(A_SLLI, dst, src2, 32-(tcgsize2size[size]*8)));
                list.concat(taicpu.op_reg_reg_const(A_SRAI, dst, dst, 32-(tcgsize2size[size]*8)));
                src2:=dst;
              end
    ...
    function tcgriscv.try_optimized_mul32_const_reg_reg(list: TAsmList; a: tcgint; src, dst: tregister) : boolean;
      var
        multiplier : dword;
        power : longint;
        bitsset : byte;
      ...
      begin
        ...
        { add up ?
          ...
          (bitsset<=popcnt(dword(nextpowerof2(multiplier,power)-multiplier) and $ffffffff)) then
          begin
            first:=true;
            while multiplier<>0 do
              begin
                shiftimm:=BsrDWord(multiplier);
                if odd(multiplier) then
                  begin
                    list.concat(taicpu.op_reg_reg_const(A_SLLI,dst,src,shiftimm));
                    list.concat(taicpu.op_reg_reg_reg(A_ADD,dst,dst,src));
                    dec(multiplier);
                  end
                else
                  if first then
                    ...
                    begin
                      tmpreg:=getintregister(list,OS_INT);
                      list.concat(taicpu.op_reg_reg_const(A_SLLI,tmpreg,src,shiftimm));
                      list.concat(taicpu.op_reg_reg_reg(A_ADD,dst,dst,tmpreg));
                    end;
                first:=false;
                dec(multiplier,1 shl shiftimm);
              end;
            if negative then
              list.concat(taicpu.op_reg_reg_reg(A_SUB,dst,NR_X0,dst));
          end
            ...
            while multiplier<>0 do
              begin
                ...
                if odd(multiplier) then
                  begin
                    list.concat(taicpu.op_reg_reg_const(A_SLLI,dst,src,shiftimm));
                    list.concat(taicpu.op_reg_reg_reg(A_SUB,dst,dst,src));
                    dec(multiplier);
                  end
                else
                  if first then
                    ...
                    begin
                      tmpreg:=getintregister(list,OS_INT);
                      list.concat(taicpu.op_reg_reg_const(A_SLLI,tmpreg,src,shiftimm));
                      list.concat(taicpu.op_reg_reg_reg(A_SUB,dst,dst,tmpreg));
                      dec(multiplier,1 shl shiftimm);
                    end;
                first:=false;
              end;
    ...
    procedure tcgriscv.a_op_const_reg_reg_checkoverflow(list: tasmlist; op: TOpCg; size: tcgsize; a: tcgint; src, dst: tregister; setflags: boolean; var ovloc: tlocation);
      var
      ...
      begin
        ...
        case op of
          ...
          OP_ADD:
            begin
              handle_reg_const_reg(list,A_ADD,src,a,hreg);
          ...
          OP_SUB:
            begin
              handle_reg_const_reg(list,A_SUB,src,a,hreg);
              if setflags and (not signed) then
          ...
          OP_MUL,OP_IMUL:
            begin
              if try_optimized_mul32_const_reg_reg(list,a,src,dst) then
                exit
              else
                begin
                  hreg:=GetIntRegister(list,OS_INT);
          ...
          OP_SAR:
            begin
              if (size in [OS_S8,OS_S16]) then
                begin
                  list.concat(taicpu.op_reg_reg_const(A_SLLI,dst,src,32-(tcgsize2size[size]*8)));
    ...
    procedure tcgriscv.a_op_reg_reg_reg_checkoverflow(list: tasmlist; op: TOpCg; size: tcgsize; src1, src2, dst: tregister; setflags: boolean; var ovloc: tlocation);
      var
        signed: boolean;
      ...
      begin
        ...
        case op of
          OP_ADD:
            begin
              list.concat(taicpu.op_reg_reg_reg(A_ADD, hreg, src2, src1));
              if setflags and (not signed) then
          ...
          OP_SUB:
            begin
              list.concat(taicpu.op_reg_reg_reg(A_SUB, hreg, src2, src1));
              if setflags and (not signed) then
              ...
              if setflags then
                begin
                  ...
                  if (op=OP_IMUL) then
                    ...
                    a_cmp_reg_reg_label(list,OS_INT,OC_EQ,hreg,NR_X0,hl);
    ...
    procedure tcgriscv.a_cmp_const_reg_label(list: tasmlist; size: tcgsize; cmp_op: topcmp; a: tcgint; reg: tregister; l: tasmlabel);
      var
        tmpreg: tregister;
      begin
        if a = 0 then
          ...
          begin
            tmpreg := GetIntRegister(list,OS_INT);
            if (a>=simm12lo) and (a<=simm12hi) and
              ...
              begin
                ...
                if cmp_op in [OC_LT,OC_B] then
                  a_cmp_reg_reg_label(list,size,OC_NE,NR_X0,tmpreg,l)
                else
                  a_cmp_reg_reg_label(list,size,OC_EQ,NR_X0,tmpreg,l);
              end
            else
              begin
                a_load_const_reg(list,OS_INT,a,tmpreg);
    ...
    procedure tcgriscv.a_cmp_reg_reg_label(list: tasmlist; size: tcgsize; cmp_op: topcmp; reg1, reg2: tregister; l: tasmlabel);
      var
        ai : Taicpu;
        op: TAsmOp;
        hreg: TRegister;
      begin
        if not (cmp_op in [OC_EQ,OC_NE]) then
          begin
            if ((reg1=NR_X0) or (reg2=NR_X0)) and (cmp_op in [OC_GT,OC_LT,OC_GTE,OC_LTE]) then
              begin
                if (reg2=NR_X0) then
                  begin
                    ai:=taicpu.op_reg_sym(A_Bcc,reg1,l);
                    ai.setcondition(TOpCmp2AsmCond_z[swap_opcmp(cmp_op)]);
                  end
                else
                  begin
                    ai:=taicpu.op_reg_sym(A_Bcc,reg2,l);
              ...
              begin
                ...
                ai:=taicpu.op_reg_reg_sym(A_Bcc,hreg,NR_X0,l);
                ai.SetCondition(TOpCmp2AsmCond_eqne[cmp_op]);
              end;
          end
        else
          begin
            ai:=taicpu.op_reg_reg_sym(A_Bcc,reg2,reg1,l);
    ...
    procedure tcgriscv.a_jmp_always(List: tasmlist; l: TAsmLabel);
      var
        ai : Taicpu;
      begin
    ...
    procedure tcgriscv.a_jmp_name(list: tasmlist; const s: string);
      begin
        List.Concat(TAiCpu.op_sym(A_J, current_asmdata.RefAsmSymbol(s)));
      end;


    procedure tcgriscv.a_jmp_flags(list: tasmlist; const f: TResFlags; l: tasmlabel);
      begin
        if f.use_const then
    ...
    procedure tcgriscv.g_flags2reg(list: tasmlist; size: tcgsize; const f: tresflags; reg: tregister);
      var
      ...
      begin
        if (f.cond in [OC_EQ,OC_NE]) then
          begin
            left:=reg;
            if f.use_const and (f.value>=-2048) and (f.value<=2047) then
              begin
                if (f.value<>0) then
            ...
            if f.cond=OC_EQ then
              ...
              list.concat(taicpu.op_reg_reg_reg(A_SLTU,reg,NR_X0,left));
          end
        else
          begin
            {
            ...
            if (f.cond in [OC_GTE,OC_LT,OC_B,OC_AE]) and
              f.use_const and
              (f.value>=simm12lo) and
              (f.value<=simm12hi) then
    ...
    procedure tcgriscv.a_mul_reg_reg_pair(list: tasmlist; size: tcgsize; src1,src2,dstlo,dsthi: tregister);
      var
        asmop: tasmop;
      begin
        {case size of
    ...
    procedure tcgriscv.g_overflowCheck(List: tasmlist; const Loc: TLocation; def: TDef);
      begin
      end;


    procedure tcgriscv.g_overflowCheck_loc(List: tasmlist; const Loc: TLocation; def: TDef; ovloc: tlocation);
      begin
      end;
    ...
    procedure tcgriscv.g_proc_entry(list: tasmlist; localsize: longint; nostackframe: boolean);
      var
      ...
      begin
        ...
        mask:=0;
        nextoffset:=0;//TMIPSProcInfo(current_procinfo).intregstart;
        ...
        if (-LocalSize >= simm12lo) and (-LocalSize <= simm12hi) then
          begin
            ...
            if assigned(framesave) then
              begin
                ...
                list.concat(Taicpu.op_reg_reg_const(A_ADDI,NR_FRAME_POINTER_REG,
                  NR_STACK_POINTER_REG,LocalSize));
              end;
          end
        else
          begin
            a_load_const_reg(list,OS_32,-LocalSize,NR_X26);
            list.concat(Taicpu.Op_reg_reg_reg(A_ADD,NR_STACK_POINTER_REG,NR_STACK_POINTER_REG,NR_X26));
            if assigned(ra_save) then
              list.concat(ra_save);
            if assigned(framesave) then
              begin
    ...
    procedure tcgriscv.g_proc_exit(list: tasmlist; parasize: longint; nostackframe: boolean);
      var
        href : treference;
        stacksize : aint;
        saveregs : tcpuregisterset;
      ...
      begin
        stacksize:=current_procinfo.calc_stackframe_size;
         if nostackframe then
           begin
             list.concat(taicpu.op_reg_reg_const(A_JALR, NR_X0, NR_X1, 0));
           end
         else
           begin
             ...
             if (-stacksize >= simm12lo) and (-stacksize <= simm12hi) then
               begin
                 list.concat(Taicpu.Op_reg_reg_const(A_ADDI, NR_STACK_POINTER_REG, NR_STACK_POINTER_REG, stacksize));
                 list.concat(taicpu.op_reg_reg_const(A_JALR, NR_X0, NR_X1, 0));
               end
             else
               begin
                 a_load_const_reg(list,OS_32,stacksize,NR_X26);
                 list.concat(taicpu.op_reg_reg_reg(A_ADD,NR_STACK_POINTER_REG,NR_STACK_POINTER_REG,NR_X26));
                 list.concat(taicpu.op_reg_reg_const(A_JALR, NR_X0, NR_X1, 0));
               end;
          end;
    ...
    procedure tcgriscv.g_concatcopy_move(list: tasmlist; const Source, dest: treference; len: tcgint);
      var
    ...
    procedure tcgriscv.g_concatcopy(list: tasmlist; const Source, dest: treference; len: tcgint);
      var
        ...
        function reference_is_reusable(const ref: treference): boolean;
          begin
            result:=(ref.base<>NR_NO) and (ref.index=NR_NO) and
               (ref.symbol=nil) and
               (ref.offset>=simm12lo) and (ref.offset+len<=simm12hi);
          end;
    ...
    procedure tcgriscv.g_concatcopy_unaligned(list: tasmlist; const Source, dest: treference; len: tcgint);
      var
        src, dst: TReference;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            list.concat(taicpu.op_reg_reg_const(A_ADDI, src.base, src.base, 1));
            list.concat(taicpu.op_reg_reg_const(A_ADDI, dst.base, dst.base, 1));
            list.concat(taicpu.op_reg_reg_const(A_ADDI, countreg, countreg, -1));
            a_cmp_reg_reg_label(list,OS_INT,OC_GT,NR_X0,countreg,lab);
          end
          else
    ...
    procedure tcgriscv.g_intf_wrapper(list: tasmlist; procdef: tprocdef; const labelname: string; ioffset: longint);
      var
        make_global: boolean;
        hsym: tsym;
        href: treference;
      ...
      begin
        ...
        case paraloc^.loc of
          LOC_REGISTER:
            begin
              if ((ioffset>=simm12lo) and (ioffset<=simm12hi)) then
                a_op_const_reg(list,OP_SUB, paraloc^.size,ioffset,paraloc^.register)
              else
                begin
                  a_load_const_reg(list, paraloc^.size, ioffset, NR_X1);
                  a_op_reg_reg(list, OP_SUB, paraloc^.size, NR_X1, paraloc^.register);
                end;
            end;
        else
        ...
        begin
          ...
          list.concat(taicpu.op_reg_ref(A_LW,NR_X26,href));

          if (procdef.extnumber=$ffff) then
            Internalerror(200006139);

          reference_reset_base(href, NR_X26, tobjectdef(procdef.struct).vmtmethodoffset(procdef.extnumber), sizeof(aint));
          list.concat(taicpu.op_reg_ref(A_LW, NR_X27, href));
          list.concat(taicpu.op_reg_reg_const(A_JALR, NR_X0, NR_X27, 0));
        end
    ...
    procedure tcgriscv64.a_load64_reg_ref(list: tasmlist; reg: tregister64; const ref: treference);
      var
        tmpref: treference;
        tmpreg: tregister;
      begin
        ...
        tmpref := ref;
        tcgriscv(cg).make_simple_ref(list,tmpref);
    ...
    procedure tcgriscv64.a_load64_ref_reg(list: tasmlist; const ref: treference; reg: tregister64);
      var
        tmpref: treference;
        tmpreg: tregister;
      begin
    ...
    procedure tcgriscv64.a_load64_ref_cgpara(list: tasmlist; const r: treference; const paraloc: tcgpara);
      var
        hreg64: tregister64;
      begin
    ...
    procedure tcgriscv64.a_op64_reg_reg(list: tasmlist; op: TOpCG; size: tcgsize; regsrc, regdst: TRegister64);
      var
        tmpreg1: TRegister;
      begin
        case op of
          ...
          OP_NOT:
            begin
              list.concat(taicpu.op_reg_reg_const(A_XOR, regdst.reglo, regsrc.reglo, -1));
              list.concat(taicpu.op_reg_reg_const(A_XOR, regdst.reghi, regsrc.reghi, -1));
            end;
        else
    ...
    procedure tcgriscv64.a_op64_const_reg(list: tasmlist; op: TOpCG; size: tcgsize; Value: int64; regdst: TRegister64);
      begin
        a_op64_const_reg_reg(list, op, size, value, regdst, regdst);
      end;


    procedure tcgriscv64.a_op64_const_reg_reg(list: tasmlist; op: TOpCG; size: tcgsize; Value: int64; regsrc, regdst: tregister64);
      var
        l: tlocation;
      begin
    ...
    procedure tcgriscv64.a_op64_reg_reg_reg(list: tasmlist; op: TOpCG; size: tcgsize; regsrc1, regsrc2, regdst: tregister64);
    var
      l: tlocation;
    begin
    ...
    procedure tcgriscv64.a_op64_const_reg_reg_checkoverflow(list: tasmlist; op: TOpCG; size: tcgsize; Value: int64; regsrc, regdst: tregister64; setflags: boolean; var ovloc: tlocation);
      var
            ...
            begin
              if lo(value)<>0 then
                begin
                  ...
                  tcgriscv(cg).handle_reg_const_reg(list,A_ADD,regsrc.reglo,aint(lo(value)),tmplo);
            ...
            begin
              ...
              if lo(value)<>0 then
                begin
                  tmplo:=cg.GetIntRegister(list,OS_32);
                  carry:=cg.GetIntRegister(list,OS_32);
                  tcgriscv(cg).handle_reg_const_reg(list,A_SUB,regsrc.reglo,aint(lo(value)),tmplo);
    ...
    procedure tcgriscv64.a_op64_reg_reg_reg_checkoverflow(list: tasmlist; op: TOpCG; size: tcgsize; regsrc1, regsrc2, regdst: tregister64; setflags: boolean; var ovloc: tlocation);
      var
