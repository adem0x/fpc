{
    ...
    Convert riscvreg.dat to several .inc files for usage with
...
begin
  ...
  assign(f,fn);
  rewrite(f);
  writeln(f,'{ don''t edit, this file is generated from riscvreg.dat }');
end;
...
procedure read_riscvreg_file;

var infile:text;

begin
   { open dat file }
   assign(infile,'riscvreg.dat');
   reset(infile);
...
begin
  { create inc files }
  openinc(confile,'rriscvcon.inc');
  openinc(supfile,'rriscvsup.inc');
  openinc(numfile,'rriscvnum.inc');
  openinc(stdfile,'rriscvstd.inc');
  openinc(gasfile,'rriscvgas.inc');
  openinc(stabfile,'rriscvsta.inc');
  openinc(dwarffile,'rriscvdwf.inc');
  openinc(norfile,'rriscvnor.inc');
  openinc(rnifile,'rriscvrni.inc');
  openinc(srifile,'rriscvsri.inc');
  openinc(grifile,'rriscvgri.inc');
  first:=true;
  for i:=0 to regcount-1 do
    begin
      if not first then
