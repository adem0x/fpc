...
INTERFACE
uses exec, utility, workbench, agraphics, tagsarray;
...
FUNCTION AllocAslRequest(reqType : ULONG; const tags: array of Const): POINTER;
FUNCTION AslRequest(requester : POINTER; const tags: array of Const): BOOLEAN;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 8));
  AllocAslRequestA := Call(reqType, tagList, AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 5));
  AllocFileRequest := Call(AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 10));
  AslRequestA := Call(requester, tagList, AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 9));
  Call(requester, AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 6));
  Call(fileReq, AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 7));
  RequestFile := Call(fileReq, AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 13));
  Call(requester, AslBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(AslBase, 14));
  Call(requester, AslBase);
end;
