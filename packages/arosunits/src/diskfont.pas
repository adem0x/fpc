...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(DiskfontBase, 6));
  Result := Call(buffer, bufBytes, flags, DiskfontBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(DiskfontBase, 8));
  Call(fontContentsHeader, DiskfontBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(DiskfontBase, 7));
  Result := Call(fontsLock, fontName, DiskfontBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(DiskfontBase, 9));
  Result := Call(sourceFont, destTextAttr, DiskfontBase);
end;
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(DiskfontBase, 5));
  Result := Call(textAttr, DiskfontBase);
end;
{
function GetDiskFontCtrl(tagid : LONGINT) : LONGINT;
type
...
var
  Call: TLocalCall;
begin
  Call := TLocalCall(GetLibAdress(DiskfontBase, XX));
  Result := Call(tagid, DiskfontBase);
end;
