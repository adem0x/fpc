...
implementation
    ...
    uses
       ...
       cutils,cclasses,
       { global }
       globtype,globals,GlobVars, cgGlobVars, tokens,verbose,constexp,
       systems,
       { symtable }
            ...
            if current_scanner.token=_ID then
             begin
               if assigned(aclass) then
                 sym:=search_class_member(aclass,current_scanner.pattern)
               else
                          ...
                          if assigned(st) then
                           begin
                             ...
                             if not(assigned(sym)) and is_object(def) then
                               sym:=search_class_member(tobjectdef(def),current_scanner.pattern);
                             if assigned(sym) then
                              begin
...
{$endif cpu64bitaddr}
          ...
          procedure parse_dispinterface(p : tpropertysym);
            ...
            begin
              ...
              if current_scanner.try_to_consume(_READONLY) then
                haswrite:=false
              else if current_scanner.try_to_consume(_WRITEONLY) then
                hasread:=false;
              ...
              if current_scanner.try_to_consume(_DISPID) then
                begin
                  pt:=comp_expr(true);
      ...
      begin
         ...
         p:=tpropertysym.create(current_scanner.orgpattern);
         ...
         if current_scanner.try_to_consume(_LECKKLAMMER) then
           begin
              ...
              repeat
                ...
                repeat
                  inc(paranr);
                  hreadparavs:=tparavarsym.create(current_scanner.orgpattern,10*paranr,varspez,generrordef,[]);
                ...
                if current_scanner.try_to_consume(_COLON) then
                  begin
                    if current_scanner.try_to_consume(_ARRAY) then
                      begin
...
{$endif cpu64bitaddr}
         ...
         if not(is_dispinterface(aclass)) then
           begin
             if current_scanner.try_to_consume(_READ) then
               begin
                 p.propaccesslist[palt_read].clear;
             ...
             if current_scanner.try_to_consume(_WRITE) then
               begin
                 p.propaccesslist[palt_write].clear;
         ...
         if assigned(aclass) and not(is_dispinterface(aclass)) and not is_classproperty then
           begin
             ...
             if current_scanner.try_to_consume(_STORED) then
              begin
                include(p.propoptions,ppo_stored);
                      ...
                      if current_scanner.idtoken<>_DEFAULT then
                       begin
                         ...
                         if (not assigned(aclass) or
                             (search_class_member(aclass,current_scanner.pattern)=nil)) and
         ...
         if current_scanner.try_to_consume(_DEFAULT) then
           begin
         ...
         else if current_scanner.try_to_consume(_NODEFAULT) then
           begin
              p.default:=longint($80000000);
           end;
(*
...
*)
         ...
         if current_scanner.try_to_consume(_IMPLEMENTS) then
           begin
    ...
    begin
      ...
      { cdecl }
      if current_scanner.try_to_consume(_CVAR) then
        begin
          current_scanner.consume(_SEMICOLON);
          is_cdecl:=true;
        end;
      ...
      { external }
      is_weak_external:=current_scanner.try_to_consume(_WEAKEXTERNAL);
      if is_weak_external or
         current_scanner.try_to_consume(_EXTERNAL) then
        begin
          is_external_var:=true;
          ...
          if not(is_cdecl) and current_scanner.try_to_consume(_NAME) then
      ...
      if current_scanner.idtoken in [_EXPORT,_PUBLIC] then
        begin
          ...
          if is_external_var then
            ...
            is_public_var:=true;
          if current_scanner.try_to_consume(_NAME) then
            C_name:=get_stringconst;
          current_scanner.consume(_SEMICOLON);
        end;
               ...
               if (current_scanner.token = _ID) then
                 begin
                   case symtablestack.top.symtabletype of
                     localsymtable :
                       vs:=tlocalvarsym.create(current_scanner.orgpattern,vs_value,generrordef,[]);
                     staticsymtable,
                     globalsymtable :
                       begin
                         vs:=tstaticvarsym.create(current_scanner.orgpattern,vs_value,generrordef,[]);
...
{$endif}
             ...
             { check for absolute }
             if current_scanner.try_to_consume(_ABSOLUTE) then
               begin
                 read_absolute(sc);
             ...
             if (current_scanner.idtoken in [_EXPORT,_EXTERNAL,_WEAKEXTERNAL,_PUBLIC,_CVAR]) then
               begin
             ...
             hintsymoptions:=[];
             deprecatedmsg:=nil;
             current_scanner.try_consume_hintdirective(hintsymoptions,deprecatedmsg);
             for i:=0 to sc.count-1 do
               begin
                 vs:=tabstractvarsym(sc[i]);
...
{$endif powerpc or powerpc64}
         ...
         while (current_scanner.token=_ID) and
            not((vd_object in options) and
                (current_scanner.idtoken in [_PUBLIC,_PRIVATE,_PUBLISHED,_PROTECTED,_STRICT])) do
           begin
...
{$endif powerpc or powerpc64}
             ...
             if (vd_object in options) then
               begin
                 ...
                 if not (vd_class in options) and current_scanner.try_to_consume(_STATIC) then
                   begin
                     current_scanner.consume(_SEMICOLON);
         ...
         if (vd_record in options) and
            current_scanner.try_to_consume(_CASE) then
           begin
              maxsize:=0;
              maxalignment:=0;
              maxpadalign:=0;
