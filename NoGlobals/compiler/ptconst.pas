...
{$endif ARM}
        ...
        begin
          ...
          if current_scanner.try_to_consume(_NIL) then
            begin
               list.concat(Tai_const.Create_sym(nil));
          ...
          current_parser.getprocvardef:=def;
          n:=comp_expr(true);
              ...
              if (srsym=nil) or
                ...
                begin
                  if (srsym=nil) then
                    ...
                    Message1(sym_e_illegal_field,sorg);
                  current_scanner.consume_all_until(_RKLAMMER);
                  break;
                end
              else
...
{$maxfpuregisters default}
    ...
    procedure read_typed_const(list:tasmlist;sym:tstaticvarsym;in_class:boolean);
      ...
      begin
        ...
        { Parse hints }
        current_scanner.try_consume_hintdirective(sym.symoptions,sym.deprecatedmsg);
