    ...
    procedure parse_parameter_dec(pd:tabstractprocdef);
      ...
      begin
        ...
        repeat
          ...
          repeat
            inc(paranr);
            vs:=tparavarsym.create(current_scanner.orgpattern,paranr*10,varspez,generrordef,[]);
          ...
          { read type declaration, force reading for value paras }
           ...
           begin
             ...
             if (current_scanner.token=_ARRAY) or
                need_array then
              begin
                current_scanner.consume(_ARRAY);
                ...
                if (target_info.system in [system_powerpc_morphos,system_m68k_amiga]) then
                  begin
                    if (current_scanner.idtoken=_LOCATION) then
                      begin
                        current_scanner.consume(_LOCATION);
    ...
    function parse_proc_head(aclass:tobjectdef;potype:tproctypeoption;var pd:tprocdef):boolean;
      ...
      begin
        ...
        if potype=potype_operator then
          begin
            sp:=overloaded_names[current_scanner.optoken];
            orgsp:=sp;
          end
        else
          begin
        ...
        if not assigned(aclass) and
           ...
           repeat
             ...
             if (srsym.typ=typesym) and
              ...
              begin
                ...
                if (current_scanner.token<>_POINT) and (potype in [potype_class_constructor,potype_class_destructor]) then
                  sp := lower(sp);
         ...
         begin
           ...
           if (not current_parser.parse_only) and
           ...
           repeat
             ...
             if not assigned(srsym) and
                not(current_parser.parse_only) and
          ...
          begin
            if (df_generic in pd._class.defoptions) then
              begin
                ...
                current_parser.parse_generic:=true;
              end;
    ...
    function parse_proc_dec(isclassmethod:boolean; aclass:tobjectdef):tprocdef;
      ...
      begin
        ...
        case current_scanner.token of
          _FUNCTION :
            begin
              current_scanner.consume(_FUNCTION);
                       ...
                       begin
                         ...
                         if assigned(pd._class) and
                           ...
                           begin
                             symtablestack.push(pd._class.symtable);
                             popclass:=true;
                             current_parser.parse_generic:=(df_generic in pd._class.defoptions);
                             old_current_objectdef:=current_objectdef;
                             current_objectdef:=pd._class;
                           end;
          ...
          _PROCEDURE :
            begin
              current_scanner.consume(_PROCEDURE);
          ...
          _CONSTRUCTOR :
            begin
              current_scanner.consume(_CONSTRUCTOR);
              if isclassmethod then
...
{$endif CPU64bitaddr}
          ...
          _DESTRUCTOR :
            begin
              current_scanner.consume(_DESTRUCTOR);
              if isclassmethod then
          ...
          _OPERATOR :
            begin
              current_scanner.consume(_OPERATOR);
              if (current_scanner.token in [first_overloaded..last_overloaded]) then
               begin
                 current_scanner.optoken:=current_scanner.token;
               end
              else
               begin
                 { Use the dummy NOTOKEN that is also declared
                   for the overloaded_operator[] }
                 current_scanner.optoken:=NOTOKEN;
                 case current_scanner.token of
                   _CARET:
                     Message1(parser_e_overload_operator_failed,'**');
                   _ID:
                     if current_scanner.idtoken = _ENUMERATOR then
                       current_scanner.optoken := _OP_ENUMERATOR
                     else
                       Message1(parser_e_overload_operator_failed,'');
                   _UNEQUAL:
                     Message1(parser_e_overload_operator_failed,'=');
              ...
              if assigned(pd) then
                begin
                  ...
                  if not current_scanner.try_to_consume(_COLON) then
                    begin
                      current_scanner.consume(_COLON);
                   ...
                   begin
                     single_type(pd.returndef,false,false);
                     if (current_scanner.optoken in [_EQUAL,_GT,_LT,_GTE,_LTE]) and
                        ((pd.returndef.typ<>orddef) or
                         (torddef(pd.returndef).ordtype<>pasbool)) then
                        Message(parser_e_comparative_operator_return_boolean);
                     if (current_scanner.optoken=_ASSIGNMENT) and
                        equal_defs(pd.returndef,tparavarsym(pd.parast.SymList[0]).vardef) then
                       message(parser_e_no_such_assignment)
                     else if not isoperatoracceptable(pd,current_scanner.optoken) then
  ...
  with tprocdef(pd) do
    begin
      ...
      { If the procedure should be imported from a DLL, a constant string follows.
        ...
        follow (FK) }
      if not(current_scanner.token=_SEMICOLON) and not(current_scanner.idtoken=_NAME) then
        begin
    ...
    function parse_proc_direc(pd:tabstractprocdef;var pdflags:tpdflags):boolean;
      ...
      { retrieve data for directive if found }
        for p:=1 to num_proc_directives do
         if proc_direcdata[p].idtok=current_scanner.idtoken then
          begin
            found:=true;
            break;
          end;
        ...
        if not found then
         begin
            ...
            if ((pdflags * [pd_procvar,pd_object,pd_objcclass,pd_objcprot])=[]) and
               not(current_scanner.idtoken=_PROPERTY) then
...
{$endif arm}
      ...
      begin
        ...
        if (po_varargs in pd.procoptions) then
         begin
           ...
           if current_parser.parse_only then
            begin
    ...
    procedure parse_proc_directives(pd:tabstractprocdef;var pdflags:tpdflags);
      ...
      begin
        ...
        while current_scanner.token in [_ID,_LECKKLAMMER] do
         begin
            ...
            begin
              ...
              if not(check_proc_directive((pd.typ=procvardef))) then
                begin
                  ...
                  if (pd_procvar in pdflags) and
                     (current_scanner.token in [_END,_RKLAMMER,_EQUAL]) then
                    break
                  else
                    begin
                      if (current_scanner.token=_COLON) then
