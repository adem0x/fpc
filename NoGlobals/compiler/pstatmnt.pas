...
implementation
    ...
    function if_statement : tnode;
      ...
      begin
         current_scanner.consume(_IF);
         ex:=comp_expr(true);
         current_scanner.consume(_THEN);
         if current_scanner.token<>_ELSE then
           if_a:=statement
         else
           if_a:=nil;

         if current_scanner.try_to_consume(_ELSE) then
            else_a:=statement
         else
           else_a:=nil;
      ...
      begin
         ...
         while current_scanner.token<>_END do
           begin
              ...
              if not current_scanner.try_to_consume(_SEMICOLON) then
                break;
              current_scanner.consume_emptystats;
           end;
         current_scanner.consume(_END);
    ...
    function case_statement : tnode;
      ...
      begin
         ...
         if (current_scanner.token in [_ELSE,_OTHERWISE]) then
           begin
              if not current_scanner.try_to_consume(_ELSE) then
                current_scanner.consume(_OTHERWISE);
    ...
    function repeat_statement : tnode;
      ...
      begin
         current_scanner.consume(_REPEAT);
         first:=nil;
         ...
         current_scanner.consume(_UNTIL);
    ...
    function while_statement : tnode;
      ...
      begin
         current_scanner.consume(_WHILE);
         p_e:=comp_expr(true);
         current_scanner.consume(_DO);
         p_a:=statement;
  ...
  {$endif not cpu64bitaddr}
    ...
    {$endif not cpu64bitaddr}
             ...
             if current_scanner.try_to_consume(_DOWNTO) then
               backward:=true
             else
               begin
                 current_scanner.consume(_TO);
                 backward:=false;
               end;
      ...
      begin
         { parse loop header }
         current_scanner.consume(_FOR);

         hloopvar:=factor(false);
         valid_for_loopvar(hloopvar,true);

         if current_scanner.try_to_consume(_ASSIGNMENT) then
           result:=for_loop_create(hloopvar)
         else if current_scanner.try_to_consume(_IN) then
           result:=for_in_loop_create(hloopvar)
         else
           current_scanner.consume(_ASSIGNMENT); // fail
      end;
...
//            p:=cwithnode.create(right,twithsymtable(withsymtable),levelcount,refnode);
    ...
    function raise_statement : tnode;
      ...
      begin
         ...
         paddr:=nil;
         pframe:=nil;
         current_scanner.consume(_RAISE);
         if not(current_scanner.token in endtokens) then
           begin
              { object }
              pobj:=comp_expr(true);
              if current_scanner.try_to_consume(_AT) then
                begin
                     ...
                     if current_scanner.token=_ID then
                       begin
                          ...
                          if current_scanner.try_to_consume(_COLON) then
                            begin
                               current_scanner.consume_sym(srsym,srsymtable);
                            ...
                            begin
                               ...
                               if srsym=nil then
                                begin
                                  current_scanner.identifier_not_found(objrealname);
                                  srsym:=generrorsym;
                                end;
                               ...
                               if srsym.typ=unitsym then
                                 begin
                                    ...
                                    if srsym=nil then
                                     begin
                                       current_scanner.identifier_not_found(current_scanner.orgpattern);
                                       srsym:=generrorsym;
                                     end;
                                    current_scanner.consume(_ID);
                                 end;
...
{$endif cpunofpu}
             if current_scanner.token<>_RECKKLAMMER then
              begin
                ...
                repeat
                  ...
                  if not current_scanner.try_to_consume(_COMMA) then
                    break;
                until false;
              end;
                  ...
                  begin
                     if current_scanner.token=_ID then
                      ...
                      begin
                        ...
                        if srsym=nil then
                          begin
                            current_scanner.identifier_not_found(current_scanner.pattern);
                            srsym:=generrorsym;
                            srsymtable:=nil;
                          end;
    ...
    function statement_block(starttoken : ttoken) : tnode;
      ...
      begin
         first:=nil;
         filepos:=current_tokenpos;
         current_scanner.consume(starttoken);

         while not(current_scanner.token in [_END,_FINALIZATION]) do
           begin
              if first=nil then
                begin
                   last:=cstatementnode.create(statement,nil);
              ...
              if (current_scanner.token in [_END,_FINALIZATION]) then
                break
              else
                begin
         ...
         { don't current_scanner.consume the finalization token, it is consumed when
...
{$endif sparc}
        ...
        current_scanner.last_endtoken_filepos:=current_tokenpos;
