...
implementation
           ...
           case current_scanner.token of
             ...
             _EQUAL:
                begin
                   ...
                   dummysymoptions:=[];
                   deprecatedmsg:=nil;
                   current_scanner.try_consume_hintdirective(dummysymoptions,deprecatedmsg);
                ...
                begin
                   ...
                   if (hdef.typ=procvardef) and
                    ...
                    begin
                      ...
                      if current_scanner.try_to_consume(_SEMICOLON) then
                       begin
           ...
           if not(current_scanner.token in [_ID,_INTCONST]) then
             ...
             begin
                if current_scanner.token=_ID then
                 symtablestack.top.insert(tlabelsym.create(current_scanner.orgpattern))
                else
                 symtablestack.top.insert(tlabelsym.create(current_scanner.pattern));
                current_scanner.consume;  //(token);
             end;
           if current_scanner.token<>_SEMICOLON then current_scanner.consume(_COMMA);
    ...
    procedure types_dec(in_class: boolean);
      ...
      procedure get_cpp_class_external_status(od: tobjectdef);
        ...
        begin
          ...
          if (current_scanner.token=_ID) and
            ...
            begin
              current_scanner.consume(_EXTERNAL);
              ...
              if (current_scanner.token=_ID) and
                 (current_scanner.idtoken=_NAME) then
                begin
      ...
      procedure get_objc_class_or_protocol_external_status(od: tobjectdef);
        begin
          ...
          if (current_scanner.token=_ID) and
            ...
            begin
              current_scanner.consume(_EXTERNAL);
              if (current_scanner.token=_ID) and
                 (current_scanner.idtoken=_NAME) and
        ...
        begin
          ...
          repeat
            if current_scanner.token=_ID then
              begin
                generictype:=ttypesym.create(current_scanner.orgpattern,cundefinedtype);
      ...
      begin
         ...
         repeat
           ...
           { generic declaration? }
           isgeneric:=current_scanner.try_to_consume(_GENERIC);
           ...
           if isgeneric then
             begin
               current_scanner.consume(_LSHARPBRACKET);
               generictypelist:=parse_generic_parameters;
               current_scanner.consume(_RSHARPBRACKET);
             end;

           current_scanner.consume(_EQUAL);

           { support 'ttype=type word' syntax }
           isunique:=current_scanner.try_to_consume(_TYPE);
           ...
           if assigned(sym) then
            begin
              if (sym.typ=typesym) then
               begin
                 if (current_scanner.token in [_CLASS,
                     _INTERFACE,
                     _DISPINTERFACE,
                     _OBJCCLASS,
                     _OBJCPROTOCOL,
           ...
           if assigned(hdef) then
            begin
              case hdef.typ of
                pointerdef :
                  begin
                    current_scanner.try_consume_hintdirective(newtype.symoptions,newtype.deprecatedmsg);
                    current_scanner.consume(_SEMICOLON);
                    if current_scanner.try_to_consume(_FAR) then
                     begin
                  ...
                  begin
                    ...
                    if istyperenaming then
                     ...
                     begin
                       ...
                       if current_scanner.try_consume_hintdirective(newtype.symoptions,newtype.deprecatedmsg) then
                         current_scanner.consume(_SEMICOLON);
                     end;
                  end;
                objectdef :
    ...
    { the top symbol table of symtablestack                }
      begin
        current_scanner.consume(_THREADVAR);
        if not(symtablestack.top.symtabletype in [staticsymtable,globalsymtable]) then
