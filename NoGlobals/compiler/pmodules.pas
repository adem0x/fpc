...
implementation
    ...
    uses
       SysUtils,
       globtype,GlobVars,cgGlobVars,version,systems,tokens,
...
{$endif ARM}
    ...
    procedure loadunits;
      var
         ...
         hp2     : tmodule;
         unitsym : tunitsym;
         //MyScanner: tscannerfile; //debug!
      begin
         current_scanner.consume(_USES);
         repeat
         ...
         //current_scanner := MyScanner; - debug!
...
{$endif debug_devirt}
      begin
         ...
         current_scanner.consume(_SEMICOLON);
         current_scanner.consume(_INTERFACE);
...
{$endif i386}
         ...
         if not current_module.interface_only then
           begin
             current_scanner.consume(_IMPLEMENTATION);
...
*)
         ...
         { finalize? }
         if not current_module.interface_only and (current_scanner.token=_FINALIZATION) then
           begin
...
{$endif debug_devirt}
    ...
    procedure proc_package;
      ...
      begin
         ...
         if current_scanner.token=_REQUIRES then
           begin
           end;
         ...
         if (current_scanner.token=_ID) and (current_scanner.idtoken=_CONTAINS) then
           begin
             ...
             while true do
               begin
                 if current_scanner.token=_ID then
                   AddUnit(current_scanner.pattern);
                 current_scanner.consume(_ID);
                 if current_scanner.token=_COMMA then
                   current_scanner.consume(_COMMA)
                 else break;
         ...
         { current_scanner.consume the last point }
