...
{$endif CPU64bitaddr}
    ...
    procedure property_dec(is_classproperty:boolean);
      ...
      begin
        ...
        if current_scanner.try_to_consume(_ENUMERATOR) then
          begin
        ...
        while current_scanner.try_consume_hintdirective(p.symoptions,p.deprecatedmsg) do
          current_scanner.Consume(_SEMICOLON);
      end;
    ...
    procedure readImplementedInterfacesAndProtocols(intf: boolean);
      var
        hdef : tdef;
      begin
        while current_scanner.try_to_consume(_COMMA) do
          begin
             id_type(hdef,false);
      ...
      begin
        if current_objectdef.objecttype in [odt_object,odt_class] then
          begin
            while true do
              begin
                if current_scanner.try_to_consume(_ABSTRACT) then
                  include(current_objectdef.objectoptions,oo_is_abstract)
                else
                if current_scanner.try_to_consume(_SEALED) then
    ...
    procedure parse_guid;
      begin
        ...
        if (current_objectdef.objecttype in [odt_interfacecom,odt_interfacecorba,odt_dispinterface]) and
           current_scanner.try_to_consume(_LECKKLAMMER) then
          begin
            readinterfaceiid;
    ...
    procedure parse_object_members;
      ...
      procedure chkobjc(pd: tprocdef);
        ...
        begin
          dummysymoptions:=[];
          deprecatedmsg:=nil;
          while current_scanner.try_consume_hintdirective(dummysymoptions,deprecatedmsg) do
            current_scanner.Consume(_SEMICOLON);
          if assigned(pd) then
            begin
      ...
      begin
        ...
        current_parser.parse_generic:=(df_generic in current_objectdef.defoptions);
          ...
          case current_scanner.token of
            ...
            _ID :
              begin
                if is_objcprotocol(current_objectdef) and
                  ...
                  begin
                    current_objectdef.symtable.currentlyoptional:=(current_scanner.idtoken=_OPTIONAL);
                    current_scanner.consume;  //(current_scanner.idtoken)
                  end
                  ...
                  _PRIVATE :
                    begin
                      if is_interface(current_objectdef) or
                       ...
                       current_scanner.consume(_PRIVATE);
                   ...
                   _PROTECTED :
                     begin
                       if is_interface(current_objectdef) or
                          is_objc_protocol_or_category(current_objectdef) then
                         Message(parser_e_no_access_specifier_in_interfaces);
                       current_scanner.consume(_PROTECTED);
                   ...
                   _PUBLIC :
                     begin
                       if is_interface(current_objectdef) or
                          is_objc_protocol_or_category(current_objectdef) then
                         Message(parser_e_no_access_specifier_in_interfaces);
                       current_scanner.consume(_PUBLIC);
                       current_objectdef.symtable.currentvisibility:=vis_public;
                       fields_allowed:=true;
                       object_member_blocktype:=bt_general;
                     end;
                   _PUBLISHED :
                     begin
                       ...
                       current_scanner.consume(_PUBLISHED);
                       current_objectdef.symtable.currentvisibility:=vis_published;
                       fields_allowed:=true;
                       object_member_blocktype:=bt_general;
                     end;
                   _STRICT :
                     begin
                       if is_interface(current_objectdef) or
                          is_objc_protocol_or_category(current_objectdef) then
                          Message(parser_e_no_access_specifier_in_interfaces);
                        current_scanner.consume(_STRICT);
                        if current_scanner.token=_ID then
                          begin
            ...
            _CLASS:
              begin
                is_classdef:=false;
                { read class method }
                if current_scanner.try_to_consume(_CLASS) then
                 begin
                   { class modifier is only allowed for procedures, functions, }
                   { constructors, destructors, fields and properties          }
                   if not(current_scanner.token in [_FUNCTION,_PROCEDURE,_PROPERTY,_VAR,_CONSTRUCTOR,_DESTRUCTOR]) then
            ...
            _END :
              ...
              current_scanner.consume(_ID); { Give a ident expected message, like tp7 }
          end;
        until false;
