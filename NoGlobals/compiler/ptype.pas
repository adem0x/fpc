...
interface
    ...
    function testcurobject : pbyte; //global variable moved into current_parser
...
implementation
    ...
    procedure generate_specialization(var tt:tdef);
      ...
      begin
        ...
        if current_parser.parse_generic then
          begin
        ...
        if current_scanner.token<>_RSHARPBRACKET then
          current_scanner.consume(_RSHARPBRACKET);
          ...
          begin
            ...
            if not err then
              begin
                ...
                current_scanner.try_to_consume(_SEMICOLON);
    ...
    { the type to allow name mangling          }
      ...
      begin
         ...
         is_unit_specific:=current_scanner.try_consume_unitsym(srsym,srsymtable,t);
         ...
         repeat
           again:=false;
             case current_scanner.token of
               ...
               _ID:
                 begin
                   if current_scanner.try_to_consume(_SPECIALIZE) then
                     begin
                       dospecialize:=true;
                       again:=true;
                     end
    ...
    function record_dec : tdef;
      ...
      begin
         ...
         { parse record }
         current_scanner.consume(_RECORD);
      ...
      var
        ...
        begin
           ...
           if not dospecialize and
              current_scanner.try_to_consume(_POINTPOINT) then
             begin
           ...
           _CARET:
                ...
                if current_scanner.token=_ARRAY then
                  ...
                  begin
                    ...
                    case current_scanner.token of
                      _CLASS :
                        begin
                          current_scanner.consume(_CLASS);
            ...
            _FUNCTION:
              begin
                ...
                if current_scanner.try_to_consume(_OF) then
                  begin
                    current_scanner.consume(_OBJECT);
