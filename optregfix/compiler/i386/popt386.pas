...
function RegUsedAfterInstruction(reg: Tregister; p: tai; var UsedRegs: TUsedRegsArray): Boolean;
var
...
begin
  ...
  RegUsedAfterInstruction :=
    (supreg in UsedRegs[regtype]) and
...
var
  ...
  UsedRegs, TmpUsedRegs: TUsedRegsArray;
  ...
  {$warning FIXME removing the first instruction fails}
    ...
    jump, but only if it's a conditional jump (PFV) }
                        if (taicpu(p).oper[1]^.typ = top_reg) and
                           ...
                           not(getsupreg(taicpu(p).oper[1]^.reg) in UsedRegs[R_INTREGISTER]) then
