...
type
  ...
  TUsedRegsArray = array[R_INTREGISTER..R_MMREGISTER] of TRegSet;
  toptreginfo = Record
...
function regLoadedWithNewValue(supreg: tsuperregister; canDependOnPrevValue: boolean;
           hp: tai): boolean;
procedure UpdateUsedRegs(var UsedRegs: tusedregsarray; p: tai);
procedure AllocRegBetween(asml: taasmoutput; reg: tregister; p1, p2: tai; const initialusedregs: tusedregsarray);
...
begin
  repeat
    ...
    while assigned(p) and
          (p.typ=ait_RegAlloc) Do
      begin
        regtype:=getregtype(tai_regalloc(p).reg);
        case tai_regalloc(p).ratype of
          ra_alloc :
            UsedRegs[regtype] := UsedRegs[regtype] + [getsupreg(tai_regalloc(p).reg)];
          ra_dealloc :
            UsedRegs[regtype] := UsedRegs[regtype] - [getsupreg(tai_regalloc(p).reg)];
        end;
        p := tai(p.next);
...
{$endif EXTDEBUG}
  ...
  { do we need this anymore? maybe for the stackpointer? (FK)
...
{$endif allocregdebug}
  if not(supreg in initialusedregs[regtype]) then
    begin
  ...
  while assigned(p1) and
    ...
    begin
      if assigned(p1.optinfo) then
        include(ptaiprop(p1.optinfo)^.usedregs[regtype],supreg);
      p1 := tai(p1.next);
      repeat
        while assigned(p1) and
...
begin
  ...
  prev := nil;
  for regtype:=low(UsedRegs) to high(UsedRegs) do
    UsedRegs[regtype] := [];
